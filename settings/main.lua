...
xi.settings.main =
{
    ...
    CAP_CURRENCY_ACCOLADES = 999999,
    CAP_CURRENCY_BALLISTA  = 20000,
    CAP_CURRENCY_SPARKS    = 999999,
    CAP_CURRENCY_VALOR     = 500000,
    ...
    EXP_RATE            = 3.000, -- Multiplies exp from script (except FoV/GoV).
    CAPACITY_RATE       = 3.000, -- Multiplies capacity points gained.
    BOOK_EXP_RATE       = 3.000, -- Multiplies exp from FoV/GoV book pages.
    ...
    STONESKIN_CAP                   = 10000,   -- Soft cap for hp absorbed by stoneskin
    BLINK_SHADOWS                   = 10,     -- Number of shadows supplied by Blink spell
    SPIKE_EFFECT_DURATION           = 1800,   -- the duration of RDM, BLM spikes effects (not Reprisal)
    ELEMENTAL_DEBUFF_DURATION       = 1200,   -- base duration of elemental debuffs
    ...
    -- NYZUL
    RUNIC_DISK_SAVE      = false, -- Relaunch: only the Runic Disc holder records floors/clears; helpers do not receive shared Mythic progress.
    ENABLE_NYZUL_CASKETS = true, -- Enable Treasure casket pops from NMs.
    ENABLE_VIGIL_DROPS   = false, -- Vigil Weapon drops retired; item IDs reserved for DAT reuse.
    ...
    DEBUG_CHOCOBO_RAISING               = true, -- true/false. Enable verbose debug logging for Chocobo Raising (visible by players).
    ...
    CHOCOBO_RAISING_DISABLE_RETIREMENT  = true, -- true/false.
    ...
    INACTIVITY_WATCHDOG_PERIOD  = 2000,  -- Log a stall after this many ms without a main-thread tick. Process is killed only if the stall lasts 15s (kWatchdogKillAfter), so a 2s DB blip can recover.
}
