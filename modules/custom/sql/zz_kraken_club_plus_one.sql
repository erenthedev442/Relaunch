...
--
-- Kraken Club +1 (19973) -- unused retail hole after Epeo/Idris 19968-19971.
-- 19972 stays a retail empty stub. A named client record at 19972 R0s the
-- client on zone-in. 19973 is cloned like Idris 19971 (0xF840 from 21070).
--
...
DELETE FROM `item_basic` WHERE `itemid` IN (19972, 19973);
INSERT INTO `item_basic`
    (`itemid`, `subid`, `name`, `sortname`, `name_jp`, `type`, `stackSize`, `flags`, `aH`, `BaseSell`)
VALUES
    (19973, 0, 'kraken_club_+1', 'kraken_club_+1', 'クラーケンクラブ+1', 7, 1, 34912, 0, 0);

DELETE FROM `item_weapon` WHERE `itemId` IN (19972, 19973);
INSERT INTO `item_weapon`
    (`itemId`, `name`, `skill`, `subskill`, `ilvl_skill`, `ilvl_parry`, `ilvl_macc`, `dmgType`, `hit`, `delay`, `dmg`, `unlock_points`)
VALUES
    (19973, 'kraken_club_+1', 11, 0, 269, 269, 228, 3, 8, 264, 16, 0);

-- jobs 4194303 = All Jobs. MId 110 matches NQ so the 3D club is unchanged.
DELETE FROM `item_equipment` WHERE `itemId` IN (19972, 19973);
INSERT INTO `item_equipment`
    (`itemId`, `name`, `level`, `ilevel`, `jobs`, `MId`, `shieldSize`, `scriptType`, `slot`, `rslot`, `rslotlook`, `su_level`)
VALUES
    (19973, 'kraken_club_+1', 99, 119, 4194303, 110, 0, 0, 3, 0, 0, 0);

DELETE FROM `item_mods` WHERE `itemId` IN (19972, 19973);
INSERT INTO `item_mods` (`itemId`, `modId`, `value`) VALUES
    (19973,  25, 25),  -- ACC
    (19973,  73,  4),  -- STORETP
    (19973, 289,  5);  -- SUBTLE_BLOW

DELETE FROM `auction_house_items` WHERE `itemid` IN (19972, 19973);
DELETE FROM `auction_house` WHERE `itemid` IN (19972, 19973);

UPDATE `char_inventory` SET `itemId` = 19973 WHERE `itemId` = 19972;
