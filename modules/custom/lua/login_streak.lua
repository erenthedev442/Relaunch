    ...
    if isLogin then
        ...
        if streak > 1 and streak < 7 then
            player:timer(4200, function(p)
                p:printToPlayer(
                    string.format('[Login Streak] %d days in a row - 7 days earns +75 marks!', streak),
                    xi.msg.channel.SYSTEM_1)
            end)
        elseif streak > 7 and streak < 14 then
            player:timer(4200, function(p)
                p:printToPlayer(
                    string.format('[Login Streak] %d days in a row - 14 days earns +200 marks!', streak),
                    xi.msg.channel.SYSTEM_1)
            end)
        elseif streak > 14 and streak < 21 then
            player:timer(4200, function(p)
                p:printToPlayer(
                    string.format('[Login Streak] %d days in a row - 21 days earns +400 marks!', streak),
                    xi.msg.channel.SYSTEM_1)
            end)
        elseif streak > 21 and streak < 30 then
            player:timer(4200, function(p)
                p:printToPlayer(
                    string.format('[Login Streak] %d days in a row - 30 days earns +750 marks!', streak),
                    xi.msg.channel.SYSTEM_1)
            end)
        end
    end
