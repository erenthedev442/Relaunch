# Cross-Job Traits

The **Cross-Job Trait Trainer** at GM Home sells permanent **passive traits** that stay active on **every job you play** — no macro, no activation, no subjob required. It's the sibling of the [Cross-Job Ability Trainer](cross-job-abilities.md): that one sells abilities you trigger with a macro, while this one sells always-on traits that just work.

!!! tip "Summary"
    Find the Trait Trainer in GM Home. Pay a flat gil price per trait (one-time, per-character). Each trait is passive and applies automatically on every job — no macro needed.

## How it works

1. **Talk to the Cross-Job Trait Trainer** at GM Home, next to the Ability Trainer.
2. **Pick a trait** from the menu and confirm the purchase.
3. **That's it** — the trait is permanently recorded to your character and is always on. It re-applies automatically every time you log in or change zones, on whatever job you're playing.

Traits stack with your gear, so they're a flat, permanent upgrade on top of whatever you equip.

<!-- DOCGEN:BEGIN id="cross-job-traits-cost" -->
Each trait costs a flat **10,000,000 gil** — a one-time, per-character purchase. Once bought, it's yours for good and works on every job, with no macro to press.
<!-- DOCGEN:END id="cross-job-traits-cost" -->

## Available traits

<!-- DOCGEN:BEGIN id="cross-job-traits-catalog" -->
_**6 traits** available — each one passive, permanent, and active on every job._

| Trait | What it does |
|---|---|
| **Dual Wield** | Wield two weapons on ANY job (+15% dual wield). |
| **Counter** | Chance to counter melee attacks (+5%). |
| **Fast Cast** | Reduce spell casting time (+15%). |
| **Attack Bonus** | Boost melee attack (+10%). |
| **Accuracy Bonus** | Boost accuracy and ranged accuracy (+20). |
| **Max HP Boost** | Increase maximum HP (+150). |
<!-- DOCGEN:END id="cross-job-traits-catalog" -->

---

<!-- DOCGEN:BEGIN id="last-updated" -->
<!-- content-hash: 9417e94ca300 -->
_Last updated: 2026-06-14 13:15 UTC_
<!-- DOCGEN:END id="last-updated" -->
