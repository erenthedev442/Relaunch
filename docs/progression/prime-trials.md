# Prime Weapon Trials

The **Prime Weapon Trials** are the gateway to a Prime weapon. Clear all five, then pay **<!--luaconst:PrimeArmory_NPC.lua:GIL_COST:comma-->750,000,000<!--/luaconst--> gil** at the [Prime Armory](prime-armory.md) on Purgonorgo Isle (`!hub`) to forge the Prime weapon of your choice — one per character.

!!! tip "Summary"
    Five trials, done in any order: gather the Abyssea collectibles, climb the Endless Tower, win a Prime Voucher from Hunting League NMs, master a job to beat a Weapon Guardian, and bank the three Aht Urhgan currencies. Finish all five, then pay **750M gil** to forge your Prime (one per character).

## The gate

<!-- DOCGEN:BEGIN id="prime-trials-gate" -->
The Prime Armory forge is locked behind the **5 Prime Weapon Trials**. They are tracked independently, so you can chip away at them **in any order** -- clear all **5** and the Prime Armory will forge the Prime weapon of your choice.

Talk to the **Prime Armory** NPC in <!--npc:prime_armory-->Purgonorgo Isle<!--/npc--> (`!hub`) at any time to see which trials you've cleared and to hand in the collection trials.
<!-- DOCGEN:END id="prime-trials-gate" -->

## The five trials

<!-- DOCGEN:BEGIN id="prime-trials-table" -->
| Trial | What it takes |
|---|---|
| **Trial 1** | 12 each of all 20 Abyssea collectibles (turn in here) |
| **Trial 2** | Endless Tower floor 50 |
| **Trial 3** | Prime Voucher (Maze Monger Crown) - rare Hunting League NM drop (turn in here) |
| **Trial 4** | Weapon Guardian defeated (Job Mastery) |
| **Trial 5** | 99 each of Jadeshell, Silverpiece & 100 Byne Bill (turn in here) |
<!-- DOCGEN:END id="prime-trials-table" -->

The rest of this page walks through each one.

## Trial 1 — The Abyssea collectibles

The grind trial. You gather the full "five elements" set of Abyssea collectibles and turn the whole lot in at the Prime Armory.

<!-- DOCGEN:BEGIN id="prime-trials-collectibles" -->
Trial 1 wants **12 of every one** of the **20 Abyssea collectibles** -- the full "five elements" set. Each element drops in **4 types** (Stone &middot; Coin &middot; Jewel &middot; Card), and you need **12 of all 20**:

| Element | Types (&times;12 each) |
|---|---|
| **Vision** | Stone &middot; Coin &middot; Jewel &middot; Card |
| **Ardor** | Stone &middot; Coin &middot; Jewel &middot; Card |
| **Wieldance** | Stone &middot; Coin &middot; Jewel &middot; Card |
| **Balance** | Stone &middot; Coin &middot; Jewel &middot; Card |
| **Voyage** | Stone &middot; Coin &middot; Jewel &middot; Card |

That's **12 &times; 20 = 240** collectibles in total. They drop from [Abyssea NMs](../endgame/abyssea-nms.md) -- bring the complete set to the Prime Armory and it consumes the lot when you turn Trial 1 in.
<!-- DOCGEN:END id="prime-trials-collectibles" -->

Because every piece drops while you farm [Abyssea NMs](../endgame/abyssea-nms.md), this trial fills in the background of normal Abyssea play. Don't hand in a partial set — the Prime Armory only accepts the complete collection, and it takes all of them at once.

## Trial 2 — Endless Tower

Reach **floor 50** of the [Endless Tower](../endgame/endless-tower.md). The Tower is a solo-friendly, ever-escalating gauntlet; floor 50 is the Prime checkpoint, and clearing it flags this trial automatically.

## Trial 3 — The Prime Voucher

Win a **Prime Voucher** — a rare drop from [Hunting League](index.md) NMs (~1%) or the custom [Maat](../endgame/maats-challenge.md) fight (0.5%) — and turn it in at the Prime Armory. It's a low-rate drop, so this trial is mostly persistence (and it stops dropping once you've cleared Trial 3). A single Voucher is all you need.

## Trial 4 — Weapon Guardian

Defeat a **Weapon Guardian**, the boss at the end of the [Job Mastery](../endgame/job-mastery.md) path. Mastering a job summons its Guardian, and beating it is the proof-of-mastery trial. These fights are deliberately brutal — bring your best setup.

## Trial 5 — Aht Urhgan currencies

The other turn-in trial. You bank **99 each of three Aht Urhgan Assault currencies** — **Lungo-Nango Jadeshell**, **Montiont Silverpiece**, and **100 Byne Bill** — and hand all 297 to the Prime Armory in one go. As with Trial 1, keep each currency as a single 99-stack in your **main inventory** (not satchel or storage) so the turn-in can take it.

## The reward

Once all five trials read complete, head to **Purgonorgo Isle** (`!hub`) and the **[Prime Armory](prime-armory.md)** will forge the Prime weapon of your choice for **<!--luaconst:PrimeArmory_NPC.lua:GIL_COST:comma-->750,000,000<!--/luaconst--> gil**.

You claim **one Prime per character**, so pick the weapon that fits your main job and go.

---

<!-- DOCGEN:BEGIN id="last-updated" -->
<!-- content-hash: 593c03c7e9fc -->
_Last updated: 2026-07-10 03:45 PDT_
<!-- DOCGEN:END id="last-updated" -->
