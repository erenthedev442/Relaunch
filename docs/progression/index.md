...
1. **Orient yourself at the hub.** Everything lives on one island plaza — **Purgonorgo Isle** (`!hub`): the economy row (Hunt Board, Gil Exchange, Sparks Exchange, Race Changer, Home Point) and the endgame row (Apex Trials, Prime Armory, Colosseum, Infamy Vendor, Augment Sage, the combat Test Dummy, and the rest), spaced out so a crowd can shop at once.
...
5. **Pick up Weekly Hunt objectives.** Visit the Weekly Hunt Board at the **Purgonorgo Isle** (`!hub`) or type `!weekly` for a status check. Five random objectives roll fresh each Monday. Sweeping all 5 objectives in a single week pays a **5,000 Hunt Marks** meta-bonus on top of the per-objective rewards. Completing these adds a big mark income boost alongside your regular NM grind.
...
10. **Augment your gear.** Visit the **Augment Moogle on Purgonorgo Isle** (`!hub`) to add random stats to equipment. For passive endgame bonuses, earn enough Infamy to unlock the **Augment Sage** — also on Purgonorgo Isle — Infamy comes from apex Escha - Ru'Aun NMs, Scheduled Invasions, and the weekly Raid. The Sage applies permanent stat bonuses outside the normal augment RNG.
