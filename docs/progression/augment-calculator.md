...
function perSlotVal(aug,roll){
  if(aug.tv)return aug.base+(tier-1); // tier-fixed (Treasure Hunter): value = your Augment Tier, no roll
...
function render(){
  ...
  const maxTotal=selAug.tv?(selAug.base+TIER_SLICES.length-1):perSlotVal(selAug,31)*MAX_SLOTS;
