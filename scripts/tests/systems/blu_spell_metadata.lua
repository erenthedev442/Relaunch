describe('BLU spell metadata integrity', function()
    ...
    it('keeps Mighty Guard as a short Unbridled window instead of a 3-minute immunity kit', function()
        local src = readFile('scripts/actions/spells/blue/mighty_guard.lua')
        assert(src:match('DURATION%s*=%s*60'), 'Mighty Guard duration must be 60s')
        assert(src:match('xi%.effect%.MIGHTY_GUARD'), 'Mighty Guard must apply its own effect')
        assert(not src:match('xi%.effect%.DEFENSE_BOOST'), 'Mighty Guard must not stack Defense Boost on Cocoon')
        assert(not src:match('xi%.effect%.HASTE,'), 'Mighty Guard must not apply a separate Haste status')
        assert(not src:match('xi%.effect%.REGEN,'), 'Mighty Guard must not apply a separate Regen 30 status')
    end)
