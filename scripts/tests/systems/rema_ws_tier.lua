...
describe('Legendary REMA native-weaponskill enhancement', function()
    ...
    it('uses the configured family bonuses and progression tuning', function()
        ...
        assert(catalog.getFamilyTuning('RELIC').ftpScale == 1.50)
        assert(catalog.getFamilyTuning('EMPYREAN').ftpScale == 2.10)
        assert(catalog.getFamilyTuning('MYTHIC').ftpScale == 2.15)
        assert(catalog.getFamilyTuning('AEONIC').ftpScale == 4.00)

        assert(catalog.getFamilyTuning('RELIC').targetDamage[1] == 400000)
        assert(catalog.getFamilyTuning('RELIC').targetDamage[2] == 700000)
        assert(catalog.getFamilyTuning('EMPYREAN').targetDamage[1] == 450000)
        assert(catalog.getFamilyTuning('EMPYREAN').targetDamage[2] == 750000)
        assert(catalog.getFamilyTuning('MYTHIC').targetDamage[1] == 500000)
        assert(catalog.getFamilyTuning('MYTHIC').targetDamage[2] == 800000)
    ...
    it('applies per-WS and family physical tuning without mutating native parameters', function()
        ...
        assert(math.abs(tuned.ftpMod[1] - 35.55) < 0.0001)
        assert(math.abs(tuned.ftpMod[3] - 35.55) < 0.0001)
        assert(tuned.atkVaries[1] == 1.15 and tuned.atkVaries[3] == 1.42)
    ...
    it('accounts for the effective WoTG Death Blossom parameters', function()
        ...
        assert(tuned.ftpMod[1] == 96.75)
        assert(tuned.ftpMod[3] == 96.75)
    ...
    it('keeps magical REMA tuning on the native stat and magic pipeline', function()
        ...
        assert(tuned.ftpMod[1] == 12.90)
        assert(tuned.ftpMod[3] == 32.25)
    ...
    it('scales Catastrophe fTP on 119 III Apocalypse', function()
        ...
        assert(math.abs(tuned.ftpMod[1] - 35.475) < 0.0001)
