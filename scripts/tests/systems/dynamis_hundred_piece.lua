...
describe('Dynamis hundred-piece', function()
    ...
    it('maps Dreamland pack singles to the matching 100-piece', function()
        assert(hundred.hundredItem(xi.zone.DYNAMIS_VALKURM, xi.item.TUKUKU_WHITESHELL)
            == xi.item.LUNGO_NANGO_JADESHELL)
        assert(hundred.hundredItem(xi.zone.DYNAMIS_BUBURIMU, xi.item.ONE_BYNE_BILL)
            == xi.item.ONE_HUNDRED_BYNE_BILL)
        assert(hundred.hundredItem(xi.zone.DYNAMIS_QUFIM, xi.item.ORDELLE_BRONZEPIECE)
            == xi.item.MONTIONT_SILVERPIECE)
        assert(hundred.hundredItem(xi.zone.DYNAMIS_TAVNAZIA, xi.item.ONE_BYNE_BILL)
            == xi.item.ONE_HUNDRED_BYNE_BILL)
    end)

    it('keeps the untagged nightmare hour fallback', function()
    ...
    it('rolls a flat 5 percent on trash and NMs', function()
        assert(hundred.CHANCE_PERCENT == 5)
        assert(hundred.NM_CHANCE_PERCENT == 5)
    end)
