...
describe('Party HP scale', function()
    ...
    it('uses the 1 / 1.5 / 2.1 / 2.7 / 3.3 / 4.0 curve and clamps', function()
        assert(scale.multiplier(1) == 1.0)
        assert(scale.multiplier(2) == 1.5)
        assert(scale.multiplier(3) == 2.1)
        assert(scale.multiplier(4) == 2.7)
        assert(scale.multiplier(5) == 3.3)
        assert(scale.multiplier(6) == 4.0)
        assert(scale.multiplier(0) == 1.0)
        assert(scale.multiplier(99) == 4.0)
    end)
    ...
    it('applies once from catalog HP and grows without shrinking', function()
        ...
        assert(mob:getMaxHP() == 21000)
        mob:setHP(10500)

        assert(scale.apply(mob, 6) == true)
        assert(mob:getMaxHP() == 40000)
        assert(mob:getHP() == 20000)
    ...
    it('re-snapshots after a later setMaxHP and reports catalog-relative current HP', function()
        ...
        mob:setHP(20000)
        scale.afterCustomHp(mob, p1)
        assert(mob:getMaxHP() == 42000)
        assert(mob:getHP() == 42000)
    end)
