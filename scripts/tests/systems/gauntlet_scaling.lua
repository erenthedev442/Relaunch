...
describe('Gauntlet damage-ceiling rebalance', function()
    ...
    it('resumes the next uncleared boss per job and resets after a full clear', function()
        ...
        assert(catalog.levelPayout(3).gil == 150000)
        assert(catalog.FINAL_REWARD.gil == 5000000)
        assert(catalog.FINAL_REWARD.pp == 500)
    end)

    it('lets a saved job rematch earlier Challenge NMs without unlocking Shinryu there', function()
        assert(catalog.maxChallengeLevel(1) == 1)
        assert(catalog.maxChallengeLevel(2) == 2)
        assert(catalog.maxChallengeLevel(7) == 7)
        assert(catalog.maxChallengeLevel(9) == 9)
        assert(catalog.maxChallengeLevel(10) == 9)
        assert(catalog.canSelectChallengeLevel(1, 1))
        assert(not catalog.canSelectChallengeLevel(1, 2))
        assert(catalog.canSelectChallengeLevel(7, 1))
        assert(catalog.canSelectChallengeLevel(7, 7))
        assert(not catalog.canSelectChallengeLevel(7, 8))
        assert(catalog.canSelectChallengeLevel(10, 2))
        assert(catalog.canSelectChallengeLevel(10, 9))
        assert(not catalog.canSelectChallengeLevel(10, 10))
        assert(not catalog.isFarmFight(1, 1))
        assert(not catalog.isFarmFight(7, 7))
        assert(catalog.isFarmFight(7, 1))
        assert(catalog.isFarmFight(10, 2))
        assert(not catalog.isFarmFight(10, 10))
        assert(catalog.challengeMenuLabel(2) == '2 Serket')
        assert(catalog.challengeMenuLabel(8) == '8 A.Virtue')
        assert(catalog.nextAfterClear(2) == 3)
    end)
