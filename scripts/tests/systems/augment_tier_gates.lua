...
describe('Augment Tier content gates', function()
    ...
    it('reserves each capped augment ceiling for T5 plus Sage 5', function()
        for cap = 1, 31 do
            local soft = xi.augmentTiers.softCeiling(cap)
            assert(xi.augmentTiers.scaleRoll(24, cap, 4, 0) < cap)
            assert(xi.augmentTiers.scaleRoll(31, cap, 5, 0) == soft)
            assert(xi.augmentTiers.scaleRoll(31, cap, 5, 4) == soft)
            assert(xi.augmentTiers.scaleRoll(31, cap, 5, 5) == cap)
        end
        assert(xi.augmentTiers.softCeiling(31) == 24)
        assert(xi.augmentTiers.scaleRoll(31, 0, 5, 5) == 0)
        assert(not xi.augmentTiers.trueMaxAllowed(5, 4))
        assert(xi.augmentTiers.trueMaxAllowed(5, 5))
        assert(xi.augmentTiers.tierFixedValue(2, 5, 4) == 8)
        assert(xi.augmentTiers.tierFixedValue(2, 5, 5) == 10)
    end)
end)
