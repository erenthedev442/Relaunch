...
--        { xi.item.SHADE_MITTENS,    800 },
 ...
 --       { xi.item.SHADE_LEGGINGS,        6000 },
    ...
    -- Puppetmaster automaton attachments -- all 117 player-facing items at
    -- 1 gil each (PUP kit enablement, same idea as !shop ninja / !shop dice).
    -- Buy here, then assign them to your automaton at a Tinkerer's Bench. IDs
    -- are the @GENERAL_TYPE (inventory) form of each attachment.
    attachments =
    {
        { 2246, 1 },  -- accelerator
        { 2249, 1 },  -- accelerator_ii
        { 9037, 1 },  -- accelerator_iii
        { 9136, 1 },  -- accelerator_iv
        { 9066, 1 },  -- amplifier
        { 9135, 1 },  -- amplifier_ii
        { 2252, 1 },  -- analyzer
        { 9072, 1 },  -- arcanic_cell
        { 9073, 1 },  -- arcanic_cell_ii
        { 9887, 1 },  -- arcanoclutch
        { 9888, 1 },  -- arcanoclutch_ii
        { 2251, 1 },  -- armor_plate
        { 2253, 1 },  -- armor_plate_ii
        { 9039, 1 },  -- armor_plate_iii
        { 9138, 1 },  -- armor_plate_iv
        { 2322, 1 },  -- attuner
        { 2262, 1 },  -- auto-repair_kit
        { 2265, 1 },  -- auto-repair_kit_ii
        { 9044, 1 },  -- auto-repair_kit_iii
        { 9143, 1 },  -- auto-repair_kit_iv
        { 3309, 1 },  -- barrage_turbine
        { 9068, 1 },  -- barrier_module
        { 3310, 1 },  -- barrier_module_ii
        { 2413, 1 },  -- coiler
        { 9041, 1 },  -- coiler_ii
        { 2352, 1 },  -- condenser
        { 2264, 1 },  -- damage_gauge
        { 9280, 1 },  -- damage_gauge_ii
        { 3314, 1 },  -- disruptor
        { 2324, 1 },  -- drum_magazine
        { 2351, 1 },  -- dynamo
        { 9069, 1 },  -- dynamo_ii
        { 9141, 1 },  -- dynamo_iii
        { 2354, 1 },  -- economizer
        ...
        { 3311, 1 },  -- galvanizer
        { 2412, 1 },  -- hammermill
        { 3307, 1 },  -- heat_capacitor
        { 9132, 1 },  -- heat_capacitor_ii
        { 2256, 1 },  -- heat_seeker
        { 2259, 1 },  -- heatsink
        { 2410, 1 },  -- ice_maker
        { 2240, 1 },  -- inhibitor
        { 9065, 1 },  -- inhibitor_ii
        { 2243, 1 },  -- loudspeaker
        { 2245, 1 },  -- loudspeaker_ii
        { 9034, 1 },  -- loudspeaker_iii
        { 9133, 1 },  -- loudspeaker_iv
        { 9881, 1 },  -- loudspeaker_v
        { 9885, 1 },  -- magniplug
        { 9886, 1 },  -- magniplug_ii
        { 2242, 1 },  -- mana_booster
        { 2327, 1 },  -- mana_channeler
        { 9233, 1 },  -- mana_channeler_ii
        { 2268, 1 },  -- mana_conserver
        { 2267, 1 },  -- mana_converter
        { 2258, 1 },  -- mana_jammer
        { 2261, 1 },  -- mana_jammer_ii
        { 9042, 1 },  -- mana_jammer_iii
        { 9142, 1 },  -- mana_jammer_iv
        { 2266, 1 },  -- mana_tank
        { 2269, 1 },  -- mana_tank_ii
        { 9045, 1 },  -- mana_tank_iii
        { 9144, 1 },  -- mana_tank_iv
        { 2353, 1 },  -- optic_fiber
        { 9234, 1 },  -- optic_fiber_ii
        { 2248, 1 },  -- pattern_reader
        { 3312, 1 },  -- percolator
        { 3308, 1 },  -- power_cooler
        { 2347, 1 },  -- reactive_shield
        { 9074, 1 },  -- regulator
        { 9067, 1 },  -- repeater
        { 2411, 1 },  -- replicator
        { 9070, 1 },  -- resister
        ...
        { 9137, 1 },  -- scope_iii
        { 9884, 1 },  -- scope_iv
        { 2250, 1 },  -- shock_absorber
        { 9038, 1 },  -- shock_absorber_ii
        { 9139, 1 },  -- shock_absorber_iii
        { 2329, 1 },  -- smoke_screen
        { 9229, 1 },  -- speedloader
        { 9230, 1 },  -- speedloader_ii
        { 2254, 1 },  -- stabilizer
        { 2257, 1 },  -- stabilizer_ii
        { 9040, 1 },  -- stabilizer_iii
        { 9140, 1 },  -- stabilizer_iv
        { 9883, 1 },  -- stabilizer_v
        { 2260, 1 },  -- stealth_screen
        { 9043, 1 },  -- stealth_screen_ii
        { 2414, 1 },  -- steam_jacket
        { 2238, 1 },  -- strobe
        { 9032, 1 },  -- strobe_ii
        { 2323, 1 },  -- tactical_processor
        { 2326, 1 },  -- target_marker
        { 2239, 1 },  -- tension_spring
        { 2241, 1 },  -- tension_spring_ii
        { 9033, 1 },  -- tension_spring_iii
        { 9131, 1 },  -- tension_spring_iv
        { 9880, 1 },  -- tension_spring_v
        { 2348, 1 },  -- tranquilizer
        { 9035, 1 },  -- tranquilizer_ii
        { 9134, 1 },  -- tranquilizer_iii
        { 9882, 1 },  -- tranquilizer_iv
        { 9889, 1 },  -- truesights
        { 2349, 1 },  -- turbo_charger
        { 2415, 1 },  -- turbo_charger_ii
        { 3313, 1 },  -- vivi-valve
        { 2416, 1 },  -- vivi-valve_ii
        { 2255, 1 },  -- volt_gun
    },
}
...
local validCategories = 'general, weapons, armor, consumables, food, dice, ammo, ninja, attachments, pets, augments'
