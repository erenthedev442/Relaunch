@echo off
setlocal
title Relaunch - Full Rebuild

REM ============================================================
REM  relaunch-rebuild.bat  -  C++ rebuild + tools + SQL + restart RELAUNCH
REM
REM  [1] Ship modules/custom + scripts + tools -> ~/relaunch/
REM  [2] Apply modules/custom/sql/*.sql to xi_relaunch DB
REM  [3-4] C++ rebuild in ~/server/build (live server stays UP during compile)
REM  [5] Copy fresh binaries from ~/server/ to ~/relaunch/
REM  [6] Restart ALL relaunch services + health check
REM  [7] Publish the relaunch website (fjb-relaunch.pages.dev) from the box
REM
REM  LIVE SERVER (~server): NOT restarted. Keeps running on old binary, so
REM  community players are never disconnected by this script.
REM
REM  This is the RELAUNCH-BRANCH copy -- run it from D:\server_relaunch.
REM  Caveat: the C++ compile (step 3-4) still runs in the box's shared
REM  ~/server build dir (relaunch + Legendary share one binary). Legendary
REM  is never restarted, so live players are unaffected, but the build dir
REM  is shared. Step [7] (website) only touches fjb-relaunch / xi_relaunch.
REM ============================================================

set "KEY=C:\Users\richa\Downloads\ffxi-server_key.pem"
set "HOST=azureuser@172.215.213.23"
set "SRC=D:\server_relaunch"
set "REMOTE=/home/azureuser/relaunch"
set "LOG=%SRC%\relaunch-rebuild.log"
set "OUT=%TEMP%\relaunch_rebuild_out.txt"
set "SSHOPT=-o StrictHostKeyChecking=accept-new"
set "SRVOK=skipped"
set "SITEOK=skipped"

(echo === Relaunch Rebuild started %DATE% %TIME% ===)> "%LOG%"

echo(
echo   RELAUNCH FULL REBUILD
echo   [1] Ship tools only  --^>  %REMOTE%/tools/
echo   [2] Apply SQL  --^>  xi_relaunch (if any)
echo   [3-4] C++ rebuild in ~/server/build  (live server stays UP during compile)
echo   [5] Copy fresh binaries to ~/relaunch/
echo   [6] Restart all relaunch services
echo   [7] Publish the relaunch website (fjb-relaunch.pages.dev)
echo   PRODUCTION SERVER (~server): NOT restarted (community players unaffected)
echo(
set "GO="
set /p GO="   Proceed? [Y/N]:  "
if /i not "%GO%"=="Y" ( echo   Cancelled - nothing changed.& goto :end )

REM ---- [1] Archive relaunch branch + ship + extract ----
echo(
echo  [1/7] Archiving relaunch branch and shipping to Azure...
(echo [%TIME%] [1/7] git-archive+upload: start)>> "%LOG%"
git -C "%SRC%" archive relaunch -- modules/custom modules/init.txt scripts tools | ssh -i "%KEY%" %SSHOPT% %HOST% "tar -xf - -C %REMOTE% --no-same-owner && echo   files-OK" > "%OUT%" 2>&1
type "%OUT%"
type "%OUT%" >> "%LOG%"
findstr /c:"files-OK" "%OUT%" >nul
if errorlevel 1 ( echo   ERROR: git archive or extract failed.& set "SRVOK=PROBLEM"& goto :finish )
(echo [%TIME%] [1/7] files shipped OK)>> "%LOG%"

REM ---- [2] Apply custom SQL to xi_relaunch ----
echo(
echo  [2/7] Applying modules/custom/sql/*.sql to xi_relaunch...
(echo [%TIME%] [2/7] sql: start)>> "%LOG%"
ssh -i "%KEY%" %SSHOPT% %HOST% "cd %REMOTE% && applied=0; shopt -s nullglob; for f in modules/custom/sql/*.sql; do sudo mariadb xi_relaunch < \"$f\" && echo \"   applied $f\" && applied=$((applied+1)); done; echo \"   sql-DONE ($applied files)\"" > "%OUT%" 2>&1
type "%OUT%"
type "%OUT%" >> "%LOG%"
findstr /c:"sql-DONE" "%OUT%" >nul
if errorlevel 1 ( echo   WARNING: SQL step may have failed - check output above.& set "SRVOK=PROBLEM"& goto :finish )
(echo [%TIME%] [2/7] sql OK)>> "%LOG%"

REM ---- [3-4] C++ rebuild on box + copy binaries to relaunch ----
echo(
echo  [3-4/7] Rebuilding C++ on box then copying binaries to relaunch...
echo          (live server stays UP during compile; only relaunch will restart)
(echo [%TIME%] [3-4/7] rebuild+copy: start)>> "%LOG%"
ssh -i "%KEY%" %SSHOPT% %HOST% "cd ~/server && echo '   [3/7] Compiling...' && cmake --build build -j2 && echo '   [4/7] Copying fresh binaries to relaunch...' && for bin in xi_map xi_connect xi_search xi_world; do if [ -f ~/server/$bin ]; then rm -f ~/relaunch/$bin && cp ~/server/$bin ~/relaunch/$bin && echo \"   copied $bin\"; fi; done && echo   binaries-OK" > "%OUT%" 2>&1
type "%OUT%"
type "%OUT%" >> "%LOG%"
findstr /c:"binaries-OK" "%OUT%" >nul
if errorlevel 1 ( echo   ERROR: build or binary copy failed - relaunch NOT restarted.& set "SRVOK=PROBLEM"& goto :finish )
(echo [%TIME%] [3-4/7] build+copy OK)>> "%LOG%"

REM ---- [5] Restart all relaunch services ----
echo(
echo  [5/7] Restarting all relaunch services (brief disconnect for relaunch players)...
(echo [%TIME%] [5/7] restart: start)>> "%LOG%"
ssh -i "%KEY%" %SSHOPT% %HOST% "sudo systemctl restart xi_connect_relaunch xi_world_relaunch xi_search_relaunch xi_map_relaunch && echo   restart-OK" > "%OUT%" 2>&1
type "%OUT%"
type "%OUT%" >> "%LOG%"
findstr /c:"restart-OK" "%OUT%" >nul
if errorlevel 1 ( echo   WARNING: restart may have failed.& set "SRVOK=PROBLEM"& goto :finish )
set "SRVOK=precheck"
(echo [%TIME%] [5/7] restart OK)>> "%LOG%"

REM ---- [6] Health check ----
echo(
echo  [6/7] Health check...
(echo [%TIME%] [6/7] health: start)>> "%LOG%"
ssh -i "%KEY%" %SSHOPT% %HOST% "for s in xi_map_relaunch xi_connect_relaunch xi_world_relaunch xi_search_relaunch; do echo    $s=$(systemctl is-active $s); done; echo '   --- recent relaunch errors ---'; sudo journalctl -u xi_map_relaunch --since '2 min ago' --no-pager 2>/dev/null | grep -iE 'attempt to|stack traceback|\.lua:[0-9]+:|\[error\]|fatal' | head -8" > "%OUT%" 2>&1
type "%OUT%"
type "%OUT%" >> "%LOG%"
findstr /c:"xi_map_relaunch=active" "%OUT%" >nul
if errorlevel 1 ( echo   WARNING: xi_map_relaunch not confirmed active.& set "SRVOK=PROBLEM" ) else if not "%SRVOK%"=="PROBLEM" ( set "SRVOK=OK" )
(echo [%TIME%] [6/7] health done, SRVOK=%SRVOK%)>> "%LOG%"

REM ---- [7] Publish the relaunch website from the box ----
REM  Builds fjb-relaunch.pages.dev from the relaunch branch + xi_relaunch DB
REM  ON THE BOX (the laptop reads an empty DB, so this MUST run box-side).
REM  Docs-only: regenerates docs -> mkdocs -> Cloudflare; does NOT touch the
REM  game services (neither relaunch nor Legendary). Reached only when steps
REM  [1]-[6] above did not hard-fail.
echo(
echo  [7/7] Publishing the relaunch website (fjb-relaunch.pages.dev) from the box...
echo        (~2-3 min: docgen -^> mkdocs -^> Cloudflare; relaunch players unaffected)
(echo [%TIME%] [7/7] site: start)>> "%LOG%"
ssh -i "%KEY%" %SSHOPT% %HOST% "bash ~/relaunch-docs/tools/refresh_site_relaunch.sh; echo; echo ===== RESULT =====; tail -14 ~/refresh_site_relaunch.log" > "%OUT%" 2>&1
type "%OUT%"
type "%OUT%" >> "%LOG%"
findstr /c:"Deployment complete" "%OUT%" >nul
if errorlevel 1 ( echo   WARNING: site publish may have failed - check output above.& set "SITEOK=PROBLEM" ) else ( set "SITEOK=OK" )
(echo [%TIME%] [7/7] site done, SITEOK=%SITEOK%)>> "%LOG%"

:finish
echo(
echo   ============================================================
echo    Relaunch rebuild finished.
echo    Server:  %SRVOK%
echo    Website: %SITEOK%
echo    Log:     %LOG%
echo    PRODUCTION SERVER (~/server, community players): NOT restarted.
echo   ============================================================
(echo [%TIME%] DONE server=%SRVOK% site=%SITEOK%)>> "%LOG%"

:end
echo(
pause
