...
def _overview(c: dict) -> str:
    ...
    return (
        ...
        "Inspect a `???` to pop one of the NMs camped there. Each pop consumes a "
        "tier-matched **Eschan pop item** — **Eschan Urn** (T1), **Eschan Cellar** "
        "(T2), or **Eschan Nef** (T3 and T4 bosses) — bought from the **Warding "
        "Circle → Pop Triggers** menu (Escha Beads). A per-player 30-min cooldown "
        "per NM also applies, so a KI plus cooldown are both required for a pop. "
