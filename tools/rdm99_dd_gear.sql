-- ============================================================================
-- RDM Lv99 Melee/Hybrid DD gear scorer  (v2 — includes latents + weapon skill/hit)
--
-- Reads:
...
--
-- Score weights default (tweak to taste):
--   STR ×2, DEX ×3, ATT ×1, ACC ×2, ATT% ×5, StoreTP ×1,
--   DW ×10, DA ×5, TA ×7, Haste ×0.05, SubBlow ×1
...
mods_always AS (
    SELECT
        ...
        SUM(CASE WHEN modId = 289 THEN value *  1    ELSE 0 END) AS score,
...
--   RDM normally swings Sword(3), Dagger(2), Club(11), Staff(12).
-- ============================================================================
WITH
mods_always AS (
    SELECT
...
WHERE     (e.jobs & 16) > 0
      ...
      -- AND w.skill IN (2, 3, 11, 12)  -- uncomment to filter to RDM weapon types
GROUP BY  e.itemId
ORDER BY  total_score DESC, e.ilevel DESC
LIMIT 30;
...
ranked AS (
    SELECT
        ...
        s.name,
        s.ilevel                                                   AS ilv,
        s.STR, s.DEX, s.ATT, s.ACC, s.ATTp, s.STP,
...
--   For an active melee RDM these are effectively always satisfied while
...
--
--   If you want to honor subjob-specific latents (e.g. /DNC, /NIN), modify
