...
CHECKS = [
    ...
    ("CP award: per-mob ceiling is 60000",
     "src/map/job_points.h", r"JOBPOINTS_CAPACITY_AWARD_MAX\s+60000", "present", 1),
    ("CP award: wide uint32 conversion input",
     "src/map/job_points.h", r"AddCapacityPoints\(uint32 amount\)", "present", 1),
    ("CP award: final mob payout uses ceiling",
     "src/map/utils/charutils.cpp", r"JOBPOINTS_CAPACITY_AWARD_MAX", "present", 1),
    ("CP farms: persistent engine respawn enabled",
     "modules/custom/lua/capacity_farm_engine.lua", r"respawn\s*=\s*catalog\.respawnSeconds", "present", 1),
    ("CP farms: dynamic entity IDs are retained",
     "modules/custom/lua/capacity_farm_engine.lua", r"releaseIdOnDisappear\s*=\s*false", "present", 1),
    ("Augments: effective player-stat ceilings retained",
     "src/map/entities/battleentity.cpp", r"case Mod::(?:ALL_SONGS_EFFECT|GILFINDER|BARRAGE_COUNT|TREASURE_HUNTER|PHANTOM_ROLL|MEDITATE_DURATION|INQUARTATA|SHIELDBLOCKRATE|MAGIC_CRITHITRATE)", "min", 9),
    ("Augments: custom Beast Affinity modifier named in core",
     "src/map/modifier.h", r"PET_BEAST_AFF\s*=\s*1200", "present", 1),
    ("Augments: equipment-only sum helper retained",
     "src/map/entities/battleentity.cpp", r"getSumGearMod\(Mod modID\)", "present", 1),
    ("Augments: pet Regen has separate 1280 ceiling",
     "src/map/entities/battleentity.cpp", r"objtype == TYPE_PET && modID == Mod::REGEN[\s\S]{0,1200}1280", "present", 1),
    ("Augments: Barrage +320 uses wide shot counter",
     "src/map/entities/battleentity.cpp", r"uint16 hitCount\s*=\s*1", "present", 1),
    ("Augments: Barrage count API is uint16",
     "src/map/utils/battleutils.h", r"uint16 getBarrageShotCount", "present", 1),
    ...
    # Nyzul: retain the solo scavenger replacement when merging upstream's
    # party-only registration/simultaneous/order lamp implementation.
    ("Nyzul: solo scavenger lamp mode exists",
     "scripts/globals/nyzul.lua", r"SCAVENGER\s*=\s*4", "present", 1),
    ("Nyzul: scavenger floors use five lamps",
     "scripts/globals/nyzul.lua", r"scavengerLampCount\s*=\s*5", "present", 1),
    ("Nyzul: floor-100 credit is tied to disk holder",
     "scripts/globals/nyzul.lua", r"diskHolder:setCharVar\('Nyzul_F100_Cleared'", "present", 1),
    ("Nyzul: visible instance countdown is started",
     "scripts/zones/Nyzul_Isle/instances/nyzul_isle_investigation.lua",
     r"player:countdown\(remaining\)", "present", 1),
    ("Nyzul: system-channel time milestones exist",
     "scripts/zones/Nyzul_Isle/instances/nyzul_isle_investigation.lua",
     r"Nyzul_TimeWarning_", "present", 1),
    ("Nyzul: system-channel objective text exists",
     "scripts/zones/Nyzul_Isle/instances/nyzul_isle_investigation.lua",
     r"Floor %d objective:", "present", 1),
