...
breaks content):
  FUNCTIONAL  shop / moogle / chocobo / storage / AH / guild / teleport /
              ...
              quest / mission / job / fame, or accepts a trade, or ROUTES its
              trigger/events into a shared xi.* service module (hunts, deeds/RoE,
              artisan, symphonic_curator, ...)  -> KEEP
...
# A script body that merely ROUTES its trigger/events into a shared xi.* module
# (xi.hunts.onTrigger, xi.deeds.validatorOnTrigger, xi.artisan.moogleOnTrigger,
# xi.symphonic_curator.onEventUpdate, ...) -- or that DEFINES a menu-option
# handler -- is interactive: the real payload (shop / reward / RoE / menu) lives
# in the MODULE, so none of the keyword regexes above fire on the local body.
# Without this, every module-delegating SERVICE npc (Records of Eminence
# validators, Monster Hunts boards, crafting/artisan moogles, the orchestrion
# curator, ...) falls straight through to FLAVOR and lands in the disable list.
# Treat delegation/handler scripts as FUNCTIONAL -> keep.
_DELEGATE = re.compile(
    r"xi\.[\w.]+\.\w*[Oo]n(?:Trigger|Event\w*|Trade)\s*\("  # routes into a module
    r"|\.onEventUpdate\s*=")                                # defines a menu handler
...
def classify(folder: str, name: str) -> str:
    ...
    try:
        ...
        return "SCENERY"
    if _FUNC.search(txt) or _DELEGATE.search(txt):
        return "FUNCTIONAL"
...
for line in NPC_LIST.read_text(encoding="utf-8", errors="replace").splitlines():
    ...
    z["total"] += 1
    if status != 0:
        # Only STATUS_TYPE::NORMAL (0) is actual visible clutter (baseentity.h).
        # 2=DISAPPEAR, 3=INVISIBLE, 6=CUTSCENE_ONLY (shown only during events),
        # etc. are already not-normally-visible -- never disable candidates, and
        # restricting to 0 makes the revert (0->2->0) trivially correct.
        z["hidden"] += 1
        continue
...
lines.append("")
lines.append(f"- **{grand['total']:,} NPCs** total; **{grand['hidden']:,} not normal-visible** "
             f"(status≠0 — already hidden, invisible, or cutscene-only); {grand['no_folder']:,} "
             f"in unmapped/system zones (skipped).")
...
# --- cutscene proposal (sibling json; SQL is generated by gen_disable_sql.py) -
# Same {folder: [npcid]} shape as npc_disable_candidates.json but for the riskier
# CUTSCENE bucket (excludes [ref]). gen_disable_sql.py folds it in on request
# (--cutscene); the default flavor-only proposal stays the conservative set. The
# disable/rollback SQL itself is generated by gen_disable_sql.py, not here.
OUT_JSON_CUT = ROOT / "tools" / "npc_audit" / "npc_disable_candidates_cutscene.json"
cut_proposal: dict[str, list[int]] = {}
for _folder, _z in zones.items():
    _cids = sorted(i for i, _p, _n, ref in _z["CUTSCENE"] if not ref)
    if _cids:
        cut_proposal[_folder] = _cids
OUT_JSON_CUT.write_text(json.dumps(cut_proposal, indent=1), encoding="utf-8")
...
print(f"Wrote {OUT_JSON_CUT}  (CUTSCENE {sum(len(v) for v in cut_proposal.values()):,} NPCs)")
