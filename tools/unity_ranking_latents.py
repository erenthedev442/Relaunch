#!/usr/bin/env python3
"""Generate modules/custom/sql/unity_ranking_latents.sql -- Relaunch's take on
retail "Unity Ranking: X+a~b" gear bonuses.
...
Relaunch the gear was silently missing its ranking line.

Owner decision (2026-09-11): NOT rank-based. Every item simply gets the MAXIMUM
value of its ranking line, always on, no Unity pledge required -- Acuity Belt
is INT+7 for everyone. Implemented as data only: one item_latents row per bonus
using the existing always-true latent JOB_LEVEL_ABOVE (51) with param 1, which
the server activates the moment the piece is equipped. No C++ involved.
...
parses the `-- <id> <name>: <bonus>` comment lines and the INSERT rows of the
emitted SQL, so keep both formats stable.
"""
...
LATENT_JOB_LEVEL_ABOVE = 51   # src/map/latent_effect.h -- PARAM: level; 1 = always active
ALWAYS = 1

# (item id, item name, mod id, mod name, retail min, retail max) -- the max is what we grant
SPEC = [
...
def render() -> str:
    ...
    out = [
        ...
        " WHERE `latentId` = %d AND `latentParam` = %d" % (LATENT_JOB_LEVEL_ABOVE, ALWAYS),
        "   AND `itemId` IN (%s);" % ", ".join(str(i) for i in ids),
    ]
    last_item = None
    for item_id, name, mod_id, mod_name, lo, hi, text in SPEC:
        ...
        out.append("INSERT INTO `item_latents` VALUES (%d, %d, %d, %d, %d);  -- %s %+d"
                   % (item_id, mod_id, hi, LATENT_JOB_LEVEL_ABOVE, ALWAYS, mod_name, hi))
    out.append("")
    return "\n".join(out)
