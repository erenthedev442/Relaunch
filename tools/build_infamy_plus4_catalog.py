...
# A few sets are cached under TWO spellings (singular vs plural-possessive) -- e.g.
# "Laksamana Tricorne +4" (head/body/feet) but "Laksamanas Gants +4" (hands/legs) --
# which splits ONE set across two menu blocks. Collapse the variants to one name.
SET_ALIASES = {
    'Laksamanas': 'Laksamana',
}

# Pieces the BG-Wiki cache is MISSING (Convokers Doublet +4 was never scraped) or
# stores WITHOUT "+4" in the name (Spaekonas Gloves, id 23988, iL119) so the
# `'+4' in name` filter skipped it -- leaving each set one slot short on the vendor.
# Force them in by id. The stat string is only the menu blurb; the real,
# server-enforced stats live in sql/item_mods.sql for these ids.
EXTRA_PIECES = [
    # (setname, slot, id, display_name, [stat line(s)])
    ('Convokers', 'body', 23954, 'Convokers Doublet +4',
     ['DEF:148 HP+89 MP+119 INT+37 MND+41 Magic Acc; Avatar: Acc/MAB; Blood Pact delay -7']),
    ('Spaekonas', 'hands', 23988, 'Spaekonas Gloves +4',
     ['DEF:104 HP+52 MP+44 INT+41 MND+43 Magic Acc; Fast Cast; Elemental magic skill']),
]
...
def title_name(name: str) -> str:
    """Title-case a piece display name so cache entries like "maxixi tiara +4"
    render as "Maxixi Tiara +4", and normalize a split set's first word via
    SET_ALIASES so "Laksamanas Gants +4" -> "Laksamana Gants +4". "+4" and
    already-capitalised words pass through."""
    out = []
    for i, w in enumerate(name.split()):
        if not w or w[0] in '+0123456789':
            out.append(w)
        else:
            cap = w[0].upper() + w[1:]
            out.append(SET_ALIASES.get(cap, cap) if i == 0 else cap)
    return ' '.join(out)
...
def splice_into_infamy_catalog(block: str) -> bool:
    """Replace the catalog.plus4Sets block in infamy_vendor_catalog.lua in place
    (brace-matched) so a regen needs no manual paste. `block` is the generated
    text from the '-- AUTO-GENERATED ...' header through the closing brace."""
    cat = REPO / 'modules' / 'custom' / 'lua' / 'infamy_vendor_catalog.lua'
    if not cat.exists():
        return False
    text = cat.read_text(encoding='utf-8')
    marker = '-- AUTO-GENERATED by tools/build_infamy_plus4_catalog.py'
    si = text.find(marker)
    if si < 0:
        print('  splice SKIPPED: marker not found; paste tools/_plus4_catalog.lua manually')
        return False
    bi = text.find('{', text.find('catalog.plus4Sets', si))
    depth, i = 0, bi
    while i < len(text):
        if text[i] == '{':
            depth += 1
    ...
    cat.write_text(new, encoding='utf-8', newline='\n')
    print(f'  spliced catalog.plus4Sets into {cat.name}')
    return True
...
def main() -> int:
    ...
    # Force-include pieces the cache lacked / mis-named (see EXTRA_PIECES).
    for xset, xslot, xid, xname, xstats in EXTRA_PIECES:
        sets[xset][xslot] = (xid, xname, xstats)
        ...
        for setname in sorted(by_job[job]):
            ...
            for slot in ['head', 'body', 'hands', 'legs', 'feet']:
                if slot in slot_entries:
                    ...
                    out_lines.append(f'            [\'{slot}\'] = {{ id = {iid}, name = \'{title_name(name)}\', stats = {{ {stat_lua} }} }},')
            out_lines.append('        } },')
        out_lines.append('    },')
    out_lines.append('}')
    out_lines.append('')
